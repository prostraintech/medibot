...
//    Serial.println("masuk");
    ...
    pendant(p_start, p_stop, p_fwd, p_rev, p_left, p_right);

    mode_rescue = digitalRead(SW_REM);
  }
...
void pendant(int p_start, int p_stop, int p_fwd, int p_rev, int p_left, int p_right) {
  
  if(p_start == 0) { p_connect = 1; }            //execute once start button pressed
  else if (p_stop ==0) {
    p_connect = 0;
    cmd= 8;
  }
  ...
  if(p_connect) {
    if(p_fwd == 0 && p_rev == 1 && p_right == 1 && p_left == 1) {
      if(cmd != 1 && cmd != 2) {
        startMillis = millis();
        cmd = 1;
        count += 1;
